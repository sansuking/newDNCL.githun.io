dncl2js= function () {
  ...
  //--------------ここから構文
	...
	var func_call = name_2byte.and(func_call_param).ret(function(_callee,_params){
		return 'this[this["func_call_cnt"]("'+_callee+'")].call(Object.create(this),'+_params.join(",")+')';
	});
	...
	var func1=name_2byte.and(func_param).and(token(/^は/)).and(program_lazy).and(token(/^を実行する/)).ret(
	...
	var func2=name_2byte.and(func_param).and(token(/^とは/)).and(program_lazy).and(token(/^である/)).ret(
		function(_name,_params,_lcb,_prog){
			var params="{"+(_params.map(function(e,i){
				return '"'+e+'":'+'arguments['+i+']';
			}).join(","))+"}";
	...
	var while_state=expression.and(token(/^の間/)).and(comma).and(program_lazy).and(token(/^を繰り返す/)).ret(function(_expr,_while,_comma,_program){
		return 'while(this["while_judge_with_inc"](__while_id__,'+_expr+")){"+_program+"}";
	});
	...
	var repeat_state1=variable.and(token(/^を/)).and(expression).and(token(/^から/)).and(expression).and(token(/^まで/)).and(expression).and(token(/^ずつ/)).and(repeat_inc.or(repeat_dec)).and(comma).and(program_lazy).and(token(/^を繰り返す/)).ret(function(_var,_1,_init,_2,_end,_3,_step,_4,_sign,_5,_program){
		/*
		 *	(function(){
		 *		var id=this["for_ent"]();
  ...
  program = statements.and(space.opt());
	...
	*/
	...
	parser.postprocess=function(str){
		console.log(str);
		//各__for_id__にidをふる
		var for_id=0,while_id=0,if_id=0;
		while(str.match(/__for_id__/))str=str.replace(/__for_id__/,for_id++);
		while(str.match(/__while_id__/))str=str.replace(/__while_id__/,while_id++);
