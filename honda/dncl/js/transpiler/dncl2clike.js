dncl2clike= function () {
  ...
  //--------------ここから構文
	...
	var arr_ref=raw_name.and(array_index).ret(function(_name,indexs){
		return {'name':_name,'toString':(function(){return _name+indexs;})};
	});
	...
	var func_call = name_2byte.and(func_call_param).ret(function(_callee,_params){
		if(isContainInJa(_callee))_callee=ja2en(_callee);
	...
	var func1=name_2byte.and(func_param).and(token(/^は/)).and(program_lazy).and(token(/^を実行する/)).ret(
		function(_name,_params,_lcb,_prog){
			return "function "+_name+"("+_params.join(",")+"){\n"+_prog+"\n}";
		}
	);
	...
	var disp_state = nl.or(expression).ret(function(_expr){
		...
		return "print"+((_noNewLineFlag)?"":"ln")+"("+_params+");";
	});
