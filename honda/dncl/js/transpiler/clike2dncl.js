clike2dncl= function () {
  ...
  // \lazies
	...
	var and=token(/^\&\&/).ret(function(){return "かつ";});
	var or=token(/^\|\|/).ret(function(){return "または";});
	var not=token(/^\!/).ret(function(){return "でない";});
	var comma=token(/^\,/);
	var semicolon=token(/^;/);
  ...
  //--------------ここから構文
	var array_index=lsb.and(expression_lazy).and(rsb).ret(function(lsb,index){return index;});
	...
	var func_call = name_2byte.and(func_call_param).ret(function(_callee,_params){
		if(isContainInEn(_callee))_callee=en2ja(_callee);
	...
	var func=token(/^function /).and(name_2byte).and(func_param).and(lcb).and(program_lazy).and(rcb).ret(
		function(_func,_name,_params,_lcb,_prog){
			return _name+"("+_params+")"+" は "+_prog+" を実行する\n";
		}
	);

	var nl=token(/^\\n/).ret(function(){return '改行';});
	var disp_param=nl.or(expression).ret(function(_param){return _param;});
	var disp_state = token(/^println/).or(token(/^print/)).ret(function(_print){
		return ((""+_print)=="println")?"表示する":"改行なしで表示する";
		...
		return _params+" を "+_printf;
	});

	var if_state_1liner=token(/^if/).and(lp).and(expression).and(rp).and(statement_lazy).ret(function(_if,_lp,_expr,_rp,_state){
		return "\nもし "+_expr+" ならば "+_state+" を実行する\n";
	});
	var if_then=token(/^if/).and(lp).and(expression).and(rp).and(lcb).and(program_lazy).and(rcb).ret(function(_if,_lp,_expr,_rp,_lcb,_program,_rcb){
		return "\nもし "+_expr+"ならば\n"+_program;
	});
	var else_if=token(/^else/).and(if_then).ret(function(_else,_elseif){
		return "\nを実行し、そうでなく "+_elseif;
	});
	var _else=token(/^else/).and(lcb).and(program_lazy).and(rcb).ret(function(_else,_lcb,_program,_rcb){
		return "\nを実行し、そうでなければ "+_program;
	});
	...
	var if_state=if_then.and(else_if.or(_else).or(_else_1liner).rep0()).ret(function(_if,_else_elseif){
		return _if+_else_elseif.join("")+" を実行する\n";
	});
	var while_state=token(/^while/).and(lp).and(expression).and(rp).and(lcb).and(program_lazy).and(rcb).ret(function(_while,_lp,_expr,_rp,_rcb,_program){
		return _expr+" の間、\n"+_program+"\nを繰り返す\n"
	});
	var repeat_state1_part1=variable.and(larrow).and(expression).ret(function(_left,_eq,_expr){
		return {'left':(""+_left),toString:function(){return _left+"を"+_expr+"から";}};
	});
	var repeat_state1_part2=variable.and(ge.or(gt).or(le).or(lt)).and(expression).ret(function(_left,_op,_expr){
		switch(""+_op){
			case ">":
				if((""+_expr).match(/^[0-9]+$/)) _expr=parseFloat(""+_expr)+1;
				else _expr="("+_expr+"+1"+")";
				break;
			case "<":
				if((""+_expr).match(/^[0-9]+$/)) _expr=parseFloat(""+_expr)-1;
		...
		return {'left':(""+_left),toString:function(){return _expr+"まで";}};
	});
	var repeat_state1_part3=variable.and(token(/^\+=/).or(token(/^-=/))).and(expression).ret(function(_left,_op,_expr){
		var str=(_op=="+=")?"増やしながら":"減らしながら";
		return {'left':(""+_left),toString:function(){return _expr+"ずつ"+str+"、\n";}};
	});
	var repeat_state1_inner=repeat_state1_part1.and(semicolon).and(repeat_state1_part2).and(semicolon).and(repeat_state1_part3).except(function(_state1,_semi1,_state2,_semi2,_state3){
		var _1=_state1.left,_2=_state2.left,_3=_state3.left;
		return (_1!==_2)||(_1!==_3)||(_2!==_3);
	}).ret(function(_state1,_semi1,_state2,_semi2,_state3){return ""+_state1+_state2+_state3;});
	var repeat_state1=token(/^for/).and(lp).and(repeat_state1_inner).and(rp).and(lcb).and(program_lazy).and(rcb).ret(function(_for,_lp,_inner,_rp,_lcb,_prgrm,_rcb){
		return ""+_inner+_prgrm+"を繰り返す\n";
	});
	var repeat_state2=token(/^repeat/).and(lp).and(expression).and(rp).and(lcb).and(program_lazy).and(rcb).ret(function(_1,_2,_num,_3,_4,_program){
		return 'ここから'+_num+"回、\n"+_program+"\nを繰り返す";
	})
	...
	var arr_init=token(/^allinit/).and(lp).and(variable).and(comma).and(expression).and(rp).and(semicolon).ret(function(_init,_lp,_variable,_comma,_expression){
		return _variable+" のすべての値を "+_expression+" にする\n";
	})
	var return_state=token(/^return /).and(expression).and(semicolon).ret(function(_1,_expr){return _expr+"を返す\n";});
	...
	var performance_test=token(/^profile/).and(lp).and(func_call.opt().ret(function(target){
		...
		return ((target)?target+"の":"")+"性能を確認する";
	})
	...
	var assigns=assign.sep1(comma,true).and(semicolon).ret(function(_assigns){ console.log(_assigns);return _assigns.join(",")+"\n";});
